...
@Component({
  selector: 'app-tipo-iden-cont-select',
  standalone: true,
  imports: [CommonModule, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => TipoIdenContSelectComponent),
      multi: true,
    },
  ],
  template: `
      ...
      @if (isOpen()) {
        <div class="absolute z-20 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          ...
          <ul class="max-h-60 overflow-y-auto py-1">
            @if (isLoading()) {
              <li class="px-3 py-2 text-sm text-gray-500">Cargando tipos...</li>
            } @else if (filteredItems().length === 0) {
              <li class="px-3 py-2 text-sm text-gray-500">No se encontraron tipos de identificación</li>
            } @else {
              @for (item of filteredItems(); track item.tipoIdenContId) {
                <li>
                  <button
                    type="button"
                    (click)="selectItem(item)"
                    class="flex w-full px-3 py-2 text-left text-sm hover:bg-blue-50"
                    [class.bg-blue-100]="value === item.tipoIdenContId"
                  >
                    <span class="font-medium text-gray-900">{{ item.tipo }}</span>
                  </button>
                </li>
              }
            }
...
export class TipoIdenContSelectComponent implements ControlValueAccessor {
  private readonly accesoriaService = inject(AccesoriaService);
  private readonly elementRef = inject(ElementRef);

  @Input() label = 'Tipo de identificación';
  @Input() required = false;
  @Input() invalid = false;
  @Input() touched = false;
  @Input() inputId = 'tipoIdenContId';
  @Input() placeholder = 'Seleccione un tipo de identificación...';

  items = signal<TipoIdenCont[]>([]);
  filteredItems = signal<TipoIdenCont[]>([]);
  selectedItem = signal<TipoIdenCont | null>(null);
  isOpen = signal(false);
  ...
  selectItem(item: TipoIdenCont): void {
    this.value = item.tipoIdenContId;
    this.selectedItem.set(item);
    this.onChange(item.tipoIdenContId);
    this.onTouched();
    this.closeDropdown();
  }
  ...
  private async loadItems(search?: string): Promise<void> {
    ...
    try {
      const items = await this.accesoriaService.listTipoIdenCont(search);
      this.items.set(items);
      this.filteredItems.set(items);
      this.syncSelectedItem();
    } catch {
      this.loadError.set('No fue posible cargar los tipos de identificación.');
      this.items.set([]);
  ...
  private syncSelectedItem(): void {
    ...
    const selected = this.items().find((item) => item.tipoIdenContId === this.value) ?? null;
