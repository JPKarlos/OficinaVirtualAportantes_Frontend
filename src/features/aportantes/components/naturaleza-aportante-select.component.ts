...
@Component({
  selector: 'app-naturaleza-aportante-select',
  standalone: true,
  imports: [CommonModule, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => NaturalezaAportanteSelectComponent),
      multi: true,
    },
  ],
  template: `
        ...
        <div class="absolute z-20 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          <div class="border-b border-gray-100 p-2">
            <input
              ...
              placeholder="Buscar naturaleza aportante..."
          ...
          <ul class="max-h-60 overflow-y-auto py-1">
            @if (isLoading()) {
              <li class="px-3 py-2 text-sm text-gray-500">Cargando naturalezas...</li>
            } @else if (filteredItems().length === 0) {
              <li class="px-3 py-2 text-sm text-gray-500">No se encontraron naturalezas de aportante</li>
            } @else {
              @for (item of filteredItems(); track item.naturalezaAportanteIde) {
                <li>
                  <button
                    type="button"
                    (click)="selectItem(item)"
                    class="flex w-full flex-col px-3 py-2 text-left text-sm hover:bg-blue-50"
                    [class.bg-blue-100]="value === item.naturalezaAportanteIde"
                  >
                    <span class="font-medium text-gray-900">
                      {{ item.nj }} - {{ item.descripcion }}
                    </span>
                  </button>
                </li>
              }
...
export class NaturalezaAportanteSelectComponent implements ControlValueAccessor {
  private readonly accesoriaService = inject(AccesoriaService);
  private readonly elementRef = inject(ElementRef);

  @Input() label = 'Naturaleza aportante';
  @Input() required = false;
  @Input() invalid = false;
  @Input() touched = false;
  @Input() inputId = 'naturalezaAportanteIde';

  items = signal<NaturalezaAportante[]>([]);
  filteredItems = signal<NaturalezaAportante[]>([]);
  selectedItem = signal<NaturalezaAportante | null>(null);
  isOpen = signal(false);
  ...
  selectedLabel(): string {
    ...
    if (!selected) {
      return 'Seleccione una naturaleza de aportante...';
    }

    return `${selected.nj} - ${selected.descripcion}`;
  }
  ...
  selectItem(item: NaturalezaAportante): void {
    this.value = item.naturalezaAportanteIde;
    this.selectedItem.set(item);
    this.onChange(item.naturalezaAportanteIde);
    this.onTouched();
    this.closeDropdown();
  }
  ...
  private async loadItems(search?: string): Promise<void> {
    ...
    try {
      const items = await this.accesoriaService.listNaturalezaAportante(search);
      this.items.set(items);
      this.filteredItems.set(items);
      this.syncSelectedItem();
    } catch {
      this.loadError.set('No fue posible cargar las naturalezas de aportante.');
      this.items.set([]);
  ...
  private syncSelectedItem(): void {
    ...
    const selected =
      this.items().find((item) => item.naturalezaAportanteIde === this.value) ?? null;
