...
@Component({
  selector: 'app-tipo-persona-select',
  standalone: true,
  imports: [CommonModule, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => TipoPersonaSelectComponent),
      multi: true,
    },
  ],
  template: `
      ...
      @if (isOpen()) {
        <div class="absolute z-20 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          ...
          <ul class="max-h-60 overflow-y-auto py-1">
            @if (isLoading()) {
              <li class="px-3 py-2 text-sm text-gray-500">Cargando tipos de persona...</li>
            } @else if (filteredItems().length === 0) {
              <li class="px-3 py-2 text-sm text-gray-500">No se encontraron tipos de persona</li>
            } @else {
              @for (item of filteredItems(); track item.tipoPersonaIde) {
                <li>
                  <button
                    type="button"
                    (click)="selectItem(item)"
                    class="flex w-full flex-col px-3 py-2 text-left text-sm hover:bg-blue-50"
                    [class.bg-blue-100]="value === item.tipoPersonaIde"
                  >
                    <span class="font-medium text-gray-900">
                      {{ item.tp }} - {{ item.descripcion }}
                    </span>
                  </button>
                </li>
              }
...
export class TipoPersonaSelectComponent implements ControlValueAccessor {
  private readonly accesoriaService = inject(AccesoriaService);
  private readonly elementRef = inject(ElementRef);

  @Input() label = 'Tipo de persona';
  @Input() required = false;
  @Input() invalid = false;
  @Input() touched = false;
  @Input() inputId = 'tipoPersonaIde';

  items = signal<TipoPersona[]>([]);
  filteredItems = signal<TipoPersona[]>([]);
  selectedItem = signal<TipoPersona | null>(null);
  isOpen = signal(false);
  ...
  selectedLabel(): string {
    ...
    return `${selected.tp} - ${selected.descripcion}`;
  }
  ...
  selectItem(item: TipoPersona): void {
    this.value = item.tipoPersonaIde;
    this.selectedItem.set(item);
    this.onChange(item.tipoPersonaIde);
    this.onTouched();
    this.closeDropdown();
  }
  ...
  private async loadItems(search?: string): Promise<void> {
    ...
    try {
      const items = await this.accesoriaService.listTipoPersona(search);
      this.items.set(items);
      this.filteredItems.set(items);
      this.syncSelectedItem();
    } catch {
      this.loadError.set('No fue posible cargar los tipos de persona.');
      this.items.set([]);
  ...
  private syncSelectedItem(): void {
    ...
    const selected =
      this.items().find((item) => item.tipoPersonaIde === this.value) ?? null;
