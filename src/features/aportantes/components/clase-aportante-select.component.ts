...
@Component({
  selector: 'app-clase-aportante-select',
  standalone: true,
  imports: [CommonModule, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => ClaseAportanteSelectComponent),
      multi: true,
    },
  ],
  template: `
        ...
        <div class="absolute z-20 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          <div class="border-b border-gray-100 p-2">
            <input
              ...
              placeholder="Buscar clase aportante..."
          ...
          <ul class="max-h-60 overflow-y-auto py-1">
            @if (isLoading()) {
              <li class="px-3 py-2 text-sm text-gray-500">Cargando clases...</li>
            } @else if (filteredItems().length === 0) {
              <li class="px-3 py-2 text-sm text-gray-500">No se encontraron clases de aportante</li>
            } @else {
              @for (item of filteredItems(); track item.claseAportanteIde) {
                <li>
                  <button
                    type="button"
                    (click)="selectItem(item)"
                    class="flex w-full flex-col px-3 py-2 text-left text-sm hover:bg-blue-50"
                    [class.bg-blue-100]="value === item.claseAportanteIde"
                  >
                    <span class="font-medium text-gray-900">
                      {{ item.clase }} - {{ item.descripcion }}
                    </span>
                  </button>
                </li>
              }
...
export class ClaseAportanteSelectComponent implements ControlValueAccessor {
  private readonly accesoriaService = inject(AccesoriaService);
  private readonly elementRef = inject(ElementRef);

  @Input() label = 'Clase aportante';
  @Input() required = false;
  @Input() invalid = false;
  @Input() touched = false;
  @Input() inputId = 'claseAportanteIde';

  items = signal<ClaseAportante[]>([]);
  filteredItems = signal<ClaseAportante[]>([]);
  selectedItem = signal<ClaseAportante | null>(null);
  isOpen = signal(false);
  ...
  selectedLabel(): string {
    ...
    if (!selected) {
      return 'Seleccione una clase de aportante...';
    }

    return `${selected.clase} - ${selected.descripcion}`;
  }
  ...
  selectItem(item: ClaseAportante): void {
    this.value = item.claseAportanteIde;
    this.selectedItem.set(item);
    this.onChange(item.claseAportanteIde);
    this.onTouched();
    this.closeDropdown();
  }
  ...
  private async loadItems(search?: string): Promise<void> {
    ...
    try {
      const items = await this.accesoriaService.listClaseAportante(search);
      this.items.set(items);
      this.filteredItems.set(items);
      this.syncSelectedItem();
    } catch {
      this.loadError.set('No fue posible cargar las clases de aportante.');
      this.items.set([]);
  ...
  private syncSelectedItem(): void {
    ...
    const selected =
      this.items().find((item) => item.claseAportanteIde === this.value) ?? null;
