...
@Component({
  selector: 'app-tipo-accion-select',
  standalone: true,
  imports: [CommonModule, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => TipoAccionSelectComponent),
      multi: true,
    },
  ],
  template: `
      ...
      @if (isOpen()) {
        <div class="absolute z-20 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          ...
          <ul class="max-h-60 overflow-y-auto py-1">
            @if (isLoading()) {
              <li class="px-3 py-2 text-sm text-gray-500">Cargando tipos de acción...</li>
            } @else if (filteredItems().length === 0) {
              <li class="px-3 py-2 text-sm text-gray-500">No se encontraron tipos de acción</li>
            } @else {
              @for (item of filteredItems(); track item.tipoAccionIde) {
                <li>
                  <button
                    type="button"
                    (click)="selectItem(item)"
                    class="flex w-full flex-col px-3 py-2 text-left text-sm hover:bg-blue-50"
                    [class.bg-blue-100]="value === item.tipoAccionIde"
                  >
...
export class TipoAccionSelectComponent implements ControlValueAccessor {
  private readonly accesoriaService = inject(AccesoriaService);
  private readonly elementRef = inject(ElementRef);

  @Input() label = 'Tipo de acción';
  @Input() required = false;
  @Input() invalid = false;
  @Input() touched = false;
  @Input() inputId = 'tipoAccionIde';

  items = signal<TipoAccion[]>([]);
  filteredItems = signal<TipoAccion[]>([]);
  selectedItem = signal<TipoAccion | null>(null);
  isOpen = signal(false);
  ...
  selectedLabel(): string {
    ...
    if (!selected) {
      return 'Seleccione un tipo de acción...';
    }
  ...
  selectItem(item: TipoAccion): void {
    this.value = item.tipoAccionIde;
    this.selectedItem.set(item);
    this.onChange(item.tipoAccionIde);
    this.onTouched();
    this.closeDropdown();
  }
  ...
  private async loadItems(search?: string): Promise<void> {
    ...
    try {
      const items = await this.accesoriaService.listTipoAccion(search);
      this.items.set(items);
      this.filteredItems.set(items);
      this.syncSelectedItem();
    } catch {
      this.loadError.set('No fue posible cargar los tipos de acción.');
      this.items.set([]);
  ...
  private syncSelectedItem(): void {
    ...
    const selected =
      this.items().find((item) => item.tipoAccionIde === this.value) ?? null;
