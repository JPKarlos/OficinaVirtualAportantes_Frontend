...
@Component({
  selector: 'app-tipo-aportante-cont-select',
  standalone: true,
  imports: [CommonModule, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => TipoAportanteContSelectComponent),
      multi: true,
    },
  ],
  template: `
        ...
        <div class="absolute z-20 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          <div class="border-b border-gray-100 p-2">
            <input
              ...
              placeholder="Buscar tipo de aportante..."
          ...
          <ul class="max-h-60 overflow-y-auto py-1">
            @if (isLoading()) {
              <li class="px-3 py-2 text-sm text-gray-500">Cargando tipos de aportante...</li>
            } @else if (filteredItems().length === 0) {
              <li class="px-3 py-2 text-sm text-gray-500">No se encontraron tipos de aportante</li>
            } @else {
              @for (item of filteredItems(); track item.tipoAportanteContIde) {
                <li>
                  <button
                    type="button"
                    (click)="selectItem(item)"
                    class="flex w-full flex-col px-3 py-2 text-left text-sm hover:bg-blue-50"
                    [class.bg-blue-100]="value === item.tipoAportanteContIde"
                  >
                    <span class="font-medium text-gray-900">
                      {{ item.ta }} - {{ item.descripcion }}
                    </span>
                  </button>
                </li>
              }
...
export class TipoAportanteContSelectComponent implements ControlValueAccessor {
  private readonly accesoriaService = inject(AccesoriaService);
  private readonly elementRef = inject(ElementRef);

  @Input() label = 'Tipo de aportante';
  @Input() required = false;
  @Input() invalid = false;
  @Input() touched = false;
  @Input() inputId = 'tipoAportanteContIde';

  items = signal<TipoAportanteCont[]>([]);
  filteredItems = signal<TipoAportanteCont[]>([]);
  selectedItem = signal<TipoAportanteCont | null>(null);
  isOpen = signal(false);
  ...
  selectedLabel(): string {
    ...
    if (!selected) {
      return 'Seleccione un tipo de aportante...';
    }

    return `${selected.ta} - ${selected.descripcion}`;
  }
  ...
  selectItem(item: TipoAportanteCont): void {
    this.value = item.tipoAportanteContIde;
    this.selectedItem.set(item);
    this.onChange(item.tipoAportanteContIde);
    this.onTouched();
    this.closeDropdown();
  }
  ...
  private async loadItems(search?: string): Promise<void> {
    ...
    try {
      const items = await this.accesoriaService.listTipoAportanteCont(search);
      this.items.set(items);
      this.filteredItems.set(items);
      this.syncSelectedItem();
    } catch {
      this.loadError.set('No fue posible cargar los tipos de aportante.');
      this.items.set([]);
  ...
  private syncSelectedItem(): void {
    ...
    const selected =
      this.items().find((item) => item.tipoAportanteContIde === this.value) ?? null;
